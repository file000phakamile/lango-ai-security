...
import { downloadComplianceExport } from "@/lib/lango/api-client";
...
export function ComplianceExport({ source }: { source: "live" | "mock" }) {
  ...
  const [lastDownloaded, setLastDownloaded] = useState<"csv" | "pdf" | null>(null);

  if (source !== "live") {
    return (
      <Panel title="Compliance Export" sub="Audit-ready CSV or PDF export for a selected date range">
        <div className="flex items-start gap-2 text-sm text-[#8A6323] bg-[#8A63231A] border border-[#8A632355] rounded-md p-3">
          <AlertTriangle size={16} className="mt-0.5 shrink-0" />
          <p>
            Compliance Export needs the live backend — there is nothing real to export from
            mock data. Start the backend (<code className="font-mono">cargo run</code>) and
            reload to use it.
          </p>
        </div>
      </Panel>
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-end gap-4 flex-wrap">
        <div>
          <label htmlFor="export-start" className="block text-xs text-[#8A93A1] mb-1">
            Start date
          </label>
          <input
            id="export-start"
        ...
        <div>
          <label htmlFor="export-end" className="block text-xs text-[#8A93A1] mb-1">
            End date
          </label>
          <input
            id="export-end"
        ...
        >
          <FileDown size={14} /> {downloading === "csv" ? "Preparing…" : "Download CSV"}
        </button>
        <button
          type="button"
        ...
        >
          <FileDown size={14} /> {downloading === "pdf" ? "Preparing…" : "Download PDF"}
        </button>
      </div>

      {downloading && (
        <p className="text-xs text-[#5B6270] mt-3 flex items-center gap-2">
          <Loader2 size={12} className="animate-spin" /> Generating your {downloading.toUpperCase()} export…
        </p>
      )}
      ...
      <p className="text-xs text-[#8A93A1] mt-4 leading-relaxed">
        The CSV contains the complete, unabridged dataset for the selected range. The PDF is a
        readable, printable summary of the same three sections (audit log, fairness metrics,
        drift history), capped at the 500 most recent audit log rows in range - use the CSV for
        the complete record.
      </p>
    </Panel>
  );
